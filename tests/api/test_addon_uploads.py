...
def test_submit_extension_with_invalid_uuid_format(base_url, session_auth):
    ...
    uuid = ['some-invalid-uuid', {'upload': 'd4ce752a971b4a5aafcd175122726431'}, None, '']
    for item in uuid:
        data = {'version': {'upload': item}}
        create_addon = requests.post(
            url=f'{base_url}{_addon_create}',
            headers={'Authorization': f'Session {session_auth}', 'Content-Type': 'application/json'},
            data=json.dumps(data)
        )
        # capture the response details to ease debugging
        print(f'For UUID "{item}": Response status is '
              f'{create_addon.status_code}; {create_addon.text}\n')
        # verify the status code and the response message for each value sent
        assert create_addon.status_code == 400, f'Actual status code was {create_addon.status_code}'
        if item is None or item == '':
            assert 'This field may not be null.' \
                   in create_addon.text, f'Actual response message was {create_addon.text}'
        else:
            assert f'“{item}” is not a valid UUID' in create_addon.text, \
...
def test_submit_extension_with_incorrect_uuid(base_url, session_auth):
    ...
    for item in uuid:
        ...
        print(f'For UUID "{item}": Response status is '
              f'{create_addon.status_code}; {create_addon.text}\n')
        assert create_addon.status_code == 400, f'Actual status code was {create_addon.status_code}'
        assert f'Object with uuid={item} does not exist.' \
               in create_addon.text, f'Actual response was {create_addon.text}'
...
def test_delete_extension_valid_token(selenium, base_url, session_auth, variables):
    ...
    assert delete_addon.status_code == 204, f'Actual status code was {delete_addon.status_code}'
    get_addon = requests.get(
        url=f'{base_url}{_addon_create}{addon}/',
        headers={'Authorization': f'Session {session_auth}'}
    )
    assert get_addon.status_code == 404, f'Actual status code was {get_addon.status_code}'
