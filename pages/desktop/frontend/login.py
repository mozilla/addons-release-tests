...
class Login(Base):
    ...
    # every FxA screen renders its single input with the same '.pb-1' class, so
    # the fields are matched by name to make sure we wait for the screen we
    # actually need instead of matching the input of the previous one
    _password_login = (By.CSS_SELECTOR, "input[name='password']")
